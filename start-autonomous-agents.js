...
class AutonomousAgentSystem {
  ...
  addInitialTasks() {
    ...
    this.cursorAgent.addTask(
      "Analyse TypeScript configuration and suggest improvements",
      {
        files: ["tsconfig.json"],
    ...
    setTimeout(() => {
      this.cursorAgent.addTask(
        "Analyse performance and provide optimisation ideas",
        {
          files: ["src/index.ts"],
  ...
  showSystemStatus() {
    ...
    console.log(
      `Cursor Agent: ${this.cursorAgent.isRunning ? "Active" : "Inactive"}`
    );
    console.log(
      `Codex Agent: ${this.codexAgent.isRunning ? "Active" : "Inactive"}`
    );
    ...
    console.log(
      `Cursor polling interval: ${this.cursorAgent.pollingInterval / 1000}s`
    );
  }
